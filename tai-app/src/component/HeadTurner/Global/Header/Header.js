...
const Header = () => {
    ...
    useEffect(() => {
        const handleClickOutside = (e) => {
            if (!searchRef.current?.contains(e.target) && !loginRef.current?.contains(e.target) && !menuRef.current?.contains(e.target) && !cartRef.current?.contains(e.target)) {
                setToggleSearch(false);
    ...
    const handleBookingClick = () => (window.location.href = "/#booking-section");
    ...
    const handleLogin = async (e) => {
        ...
        try {
            const res = await fetch("https://6871fc0376a5723aacd3429e.mockapi.io/info");
        ...
        <header className={`header ${scrolled ? "scrolled" : ""}`}>
            ...
            <div className="actions">
                ...
                <div className="search" ref={searchRef}>
                    <i className="fa-solid fa-magnifying-glass" onClick={() => handleToggle("search")}></i>
                    <div className={`search-popup ${toggleSearch ? "active" : ""}`}>
                        <h4 className="search-title">TÌM KIẾM</h4>
                        <div className="search-box">
                            <input type="text" placeholder="Tìm kiếm sản phẩm..." value={searchTerm} onChange={handleChangeSearchTerm} onKeyDown={(e) => e.key === "Enter" && handleSearch()} />
                                        ...
                                        >
                                            <div className="suggestion-left">
                                                ...
                                                <div className="suggestion-text">
                                                    <div className="suggestion-name">{highlightMatch(item.name, searchTerm)}</div>
                                                    {item.description && <div className="suggestion-desc">{highlightMatch(item.description, searchTerm)}</div>}
                                                </div>
                                                <div className="suggestion-price">{Number(item.price).toLocaleString("vi-VN")} ₫</div>
                                            </div>
                                        </Link>
                                    </li>
                                ))}
                ...
                {user ? (
                    ...
                    <div className="login" ref={loginRef}>
                        <div className={`login-toggle ${toggleLogin ? "active" : ""}`}>
                        ...
                        <i className="fas fa-user" onClick={() => handleToggle("login")}></i>
                    </div>
                )}

                {/* Cart */}
                <div className="cart-left" onClick={() => handleToggle("cart")} ref={cartRef}>
                    <i className="fas fa-shopping-cart"></i>
                    <div className="cart-count">{cartItems.length}</div>
                    <CartPopup visible={showCart} onClose={() => setShowCart(false)} />
                </div>
                                ...
                                {availableServices.map((s, i) => (
                                    <div key={i} className="booked-item1">
                                        ...
                                        <div className="service-info">
                                            ...
                                            <div className="service-price">{Number(s.price).toLocaleString("vi-VN")} ₫</div>
                                        </div>
                ...
                <div className="icon-menu" ref={menuRef}>
                    <i className="fa-solid fa-bars" onClick={() => handleToggle("menu")}></i>
                    <ul className={`nav-menu-icon ${toggleMenu ? "active" : ""}`}>
                        <li>
                            <Link to="/" onClick={scrollToTop}>
                                Trang chủ
                            </Link>
